...
const Points = () => {
  ...
  useEffect(()=>{
    async function loadPosition(){
      ...
      if(status!=='granted'){
          Alert.alert('Precisamos de sua permissão para localizaçaõ')
          return;
      }
  ...
  function handleSelectItem(id: number) {
    const alredy = selectedItems.findIndex(item => item === id)
    if (alredy >= 0) {
      ...
      <View style={styles.itemsContainer}>
        ...
        >
          {items.map(e => (
            ...
            key={String(e.id)}style={[
            styles.item,
            selectedItems.includes(e.id)? styles.selectedItem: {}]} 
            onPress={()=>handleSelectItem(e.id)}>
